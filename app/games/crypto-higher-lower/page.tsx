...
export default function CryptoHigherLowerPage() {
  ...
  const loadNextPair = useCallback(
    async (nextRound: number, curA: TokenMeta | null, curPriceA: number, curMcapA: number) => {
      ...
      setTokenA(a!); setTokenB(newB!);
      setPriceA(pa); setPriceB(pb);
      setMcapA(ma);  setMcapB(mb);
      setRound(nextRound);
  ...
  const startGame = useCallback(async () => {
    ...
    setScore(0); setStreak(0); setHistory([]);
    setFlashResult(null); setFetchError(null);
    setTokenA(null); setTokenB(null);
    setStartTxHash(undefined); setEndTxHash(undefined);
    ...
    if (mode === "onchain" && contractAddress) {
      try {
        ...
        const diffIdx = difficulty === "easy" ? 0 : difficulty === "medium" ? 1 : 2;
  ...
  const handleGuess = useCallback(
    async (guess: "higher" | "lower") => {
      ...
      const result: RoundResult = { round, tokenA, tokenB, priceA, priceB, guess, correct: isCorrect, pts, walletBonus: wb };
      setHistory((h) => {
    ...
    [tokenA, tokenB, gameStatus, isGuessing, priceA, priceB, mcapB, streak, difficulty, round, score, holdsTokenB, loadNextPair, mode, contractAddress, writeContractAsync]
  );
      ...
      <div className="max-w-xl mx-auto">
        ...
        {gameStatus === "idle" && (
          <div className="rounded-2xl bg-white/5 border border-white/10 p-6 mb-5">
            <h2 className="text-white font-bold text-center mb-4">{t("games.cryptohigherlower.difficulty") || "Choose Difficulty"}</h2>
            <DifficultyPicker
              value={difficulty}
            ...
            <div className="mt-5 space-y-2">
              <p className="text-gray-400 text-xs font-bold uppercase tracking-wider">{t("games.cryptohigherlower.howToPlay") || "How to Play"}</p>
              {(["rule1","rule2","rule3","rule4","rule5"] as const).map((key, i) => (
                <p key={key} className="text-gray-400 text-xs flex gap-2">
                  <span className="font-bold flex-shrink-0" style={{ color: "#FCFF52" }}>{i + 1}.</span>
                  {t(`games.cryptohigherlower.${key}`) || ""}
                </p>
              ))}
            </div>
          </div>
        ...
        {gameStatus === "playing" && tokenA && tokenB && (
          <>
              ...
              <AnimatePresence>
                {flashResult && (
                  <ResultFlash
                    ...
                    labelWrong={t("games.cryptohigherlower.gameOver") || "Wrong!"}
            ...
            {fetchError && <p className="text-red-400 text-xs text-center mb-3">{fetchError}</p>}
          </>
        )}
        ...
        {gameStatus === "gameover" && (
          ...
          >
            <div className="text-center mb-5">
              ...
              <p className="text-gray-400 text-sm mt-1">
                {roundsWon === MAX_ROUNDS
                  ...
                  : `${roundsWon} / ${MAX_ROUNDS} ${t("games.cryptohigherlower.correct") || "correct"}`}
              </p>
            </div>

            <div className="rounded-xl bg-white/5 border border-white/10 p-4 mb-5 space-y-2 text-sm">
              <p className="text-gray-400 text-xs font-bold uppercase tracking-wider mb-2">{t("games.cryptohigherlower.scoreBreakdown") || "Score Breakdown"}</p>
              ...
              {difficulty === "hard" && (
                <div className="flex justify-between">
                  ...
                  <span className="font-bold" style={{ color: "#E84142" }}>×1.5</span>
                </div>
              )}
              {walletBonusTotal > 0 && (
                <div className="flex justify-between">
                  <span className="text-gray-300">{t("games.cryptohigherlower.walletBonus") || "💼 Wallet holder bonus"}</span>
            ...
            {history.length > 0 && (
              <div className="mb-5">
                <p className="text-gray-400 text-xs font-bold uppercase tracking-wider mb-2">{t("games.cryptohigherlower.roundHistory") || "Round History"}</p>
            ...
            {mode === "onchain" && endTxHash && (
              <div className="mb-4 p-3 rounded-xl bg-green-900/20 border border-green-500/30 text-center">
                <p className="text-green-400 text-xs font-semibold">{t("games.cryptohigherlower.onChainRecorded") || "✅ Result recorded on-chain"}</p>
                {explorerUrl && (
                  <a href={explorerUrl} target="_blank" rel="noopener noreferrer"
                    className="text-green-400 text-xs underline hover:text-green-300"
                  >
                    {t("games.cryptohigherlower.viewOnExplorer") || `View on ${explorerName} →`}
                  </a>
                )}
              </div>
            )}
        ...
        {gameStatus === "playing" && (
          <div className="rounded-2xl bg-white/5 border border-white/10 p-4 mb-5">
            <div className="grid grid-cols-3 gap-3 text-center">
              ...
              <div>
                <p className="text-gray-400 text-xs uppercase mb-1">{t("games.cryptohigherlower.multiplier") || "Mult"}</p>
        ...
        <div className="text-center text-xs text-gray-600 mb-6">
          <p>{t("games.cryptohigherlower.footer") || "Prices via CoinGecko · refreshed every 60s"}</p>
          {mode === "onchain" && explorerUrl ? (
            ...
            <span>{t("chain.comingSoon") || "On-chain coming soon"}</span>
          )}
        </div>
