...
export function PlayerHand({ cards, total }: { cards: Card[]; total: number }) {
  ...
  return (
    <div className="text-center">
      <h3 className="text-gray-900 dark:text-white text-lg font-bold mb-3">{t('games.blackjack.yourHand')}</h3>
...
export function DealerHand({ cards, total, hideFirstCard }: { cards: Card[]; total: number; hideFirstCard?: boolean }) {
  ...
  return (
    <div className="text-center">
      <h3 className="text-gray-900 dark:text-white text-lg font-bold mb-3">{t('games.blackjack.dealer')}</h3>
      <div className="flex justify-center gap-2 mb-3">
        {cards.map((card, idx) => (
          <CardDisplay
            key={idx}
